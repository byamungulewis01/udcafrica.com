...
const submitForm = () => {
    if (isEditMode.value) {
        form.put(route('blog-categories.update', form.id), {
            preserveScroll: true,
            onSuccess: () => {
                showDialog.value = false;
                resetForm();
                router.visit(route('blog-categories.index'), {
                    only: ['categories']
                });
            },
        });
...
const confirmDelete = (categoryId) => {
    if (confirm('Are you sure you want to delete this category?')) {
        useForm().delete(route('blog-categories.destroy', categoryId), {
            preserveScroll: true,
            onSuccess: () => {
                router.visit(route('blog-categories.index'), {
                    only: ['categories']
...
const moveItem = async (index, direction) => {
    ...
    try {
        await axios.post(route('blog-categories.reorder'), {
        ...
        <Card>
            <CardHeader class="pb-3">
                <div class="flex justify-between items-center">
                    <div>
                        <CardTitle class="text-2xl">Blog Categories</CardTitle>
